({
    ...
    filterOptionsHelper : function(component) {
        ...
        var searchOnVal = component.get('v.searchOnVal');
        if(searchText.length >= minChar) {
            var flag = true;
            console.log('msbox-searchonVal-->', searchOnVal,searchText);
            if (searchOnVal) {
                options.forEach( function(element,index) {                
                    if(element.value.toLowerCase().trim().includes(searchText.toLowerCase().trim())) {
   ...
   selectItemHelper: function(component, event) {
    ...
    var compEvent = component.getEvent("cloneQuoteEvent");
    if (compEvent) {
        compEvent.setParams({
            "selectedValue": multiSelect ? values : value
        });
        compEvent.fire();
    }
